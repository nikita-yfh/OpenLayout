...
void Settings::Save(File &file) const {
	...
	file.Write<uint16_t>(windowSize.width());
	file.Write<uint16_t>(windowSize.height());
...
void Settings::Load(File &file) {
	...
	windowPos.rx() = file.Read<uint16_t>();
	windowPos.ry() = file.Read<uint16_t>();
	windowSize.rwidth() = file.Read<uint16_t>();
	windowSize.rheight() = file.Read<uint16_t>();
